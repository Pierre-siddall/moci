...
def tstmsk(config, modl, isec, item, h_vers, exppxi):
    ...
    l_casim = False # Hard coded in mphys_inputs_mod.F90 for now.
#    l_casim = _get_var(config, "namelist:run_precip",
#                         "l_casim", type_="logical")
    ...
    l_endgame = _get_var(config, "namelist:run_dyn", "l_endgame",
                        default=False, type_="logical")
    ...
    if (nsec == 0 or nsec == 31 or nsec == 32 or  nsec == 33 or
        nsec == 36 or nsec == 37):
        if sum_iopn != 0:
            ...
            if (n2n1 == 99 and nsec != 33 and nsec != 36 and
                ...
                elif n11 == 5 and not (l_albedo_ops and l_spec_albedo):
                # VIS and NIR albedo_obs
                    ...
                    msg = _form_msg("n12", n12,
                           "PC2 cloud fraction boundary values out")
...
#                           "CASIM microphysics is switched off")
    ...
    # PLEASE NOTE, in tstmsk_ukca_mod all records are initialized
    # to false (contradictory to the logic in tstmsk).
    # This code has to replicate logic of tstmsk_ukca, assuming
    # that all records are initialised to true at the beginning.
    ...
    if nsec == 34 or nsec == 51:
        if sum_iopn == 0:
            ...
            l_ukca_ageair = _get_var(config, "namelist:run_ukca",
                    "l_ukca_ageair", default=False, type_="logical")
            ...
            if i_ukca_chem == 11:
                ...
                if not (n8 == 1 and l_ukca_chem_aero):
                    l_mask = False
                    msg = _form_msg("n8", n8, "Offline oxidants BE")
            elif i_ukca_chem == 50:
                if not (n4 == 1 or (n4 == 2 and l_ukca_chem_aero)
                        or (n4 == 3 and l_ukca_new_emiss)):
                    l_mask = False
                    msg = _form_msg("n4", n4, "Tropospheric plus isoprene")
            elif i_ukca_chem == 51:
                if not (n5 == 1 or (n5 == 2 and l_ukca_chem_aero)
                        or (n5 == 3 and l_ukca_new_emiss)):
                    l_mask = False
                    msg = _form_msg("n5", n5, 
                       "Stratospheric plus tropospheric chemistry")
            elif i_ukca_chem == 52:
                if not (n6 == 1 or (n6 == 2 and l_ukca_chem_aero)
                        or (n6 == 3 and l_ukca_new_emiss)):
                    l_mask = False
                    msg = _form_msg("n6", n6, "Standard stratospheric chemistry")
            elif i_ukca_chem == 54:
                if not ((n7 == 1 and l_ukca_chem_aero)
                        or (n7 == 3 and l_ukca_new_emiss)):
                    l_mask = False
                    msg = _form_msg("n7", n7, "Offline oxidants N-R")
            elif (i_ukca_chem != 11 and
                  i_ukca_chem != 13 and i_ukca_chem != 14 and
                  i_ukca_chem != 50 and i_ukca_chem != 51 and
                  i_ukca_chem != 52 and i_ukca_chem != 54 and 
                  not l_ukca_ageair):
                l_mask = False
                msg = "Sec 34, 51: unknown chemistry scheme code"
            elif int(iopn[0:24]) > 0:
                l_mask = False
                msg = "Sec 34, 51: unknown option code"
        ...
        # Controlled by the variable i_mode_setup
            ...
            if not l_ukca_mode:
                ...
                elif i_mode_setup == 6 and n6 != 1:
                    l_mask = False
                    msg = _form_msg("n6", n6, "i_mode_setup is 6")                    
                elif i_mode_setup > 2 and i_mode_setup != 6:
                    l_mask = False
                    msg = "Sec 34, 51: unknown i_mode_setup code"
                if int(iopn[1:24]) > 0 or int(iopn[26:28]) > 0:
                    l_mask = False
                    msg = "Sec 34, 51: unknown option code"

    if nsec == 50 or nsec == 52:
        if sum_iopn == 0:
            # always available for all zeroes
            pass
        elif n30 == 0:
            i_ukca_chem = _get_var(config, "namelist:run_ukca",
                       "i_ukca_chem", type_="integer")
            if i_ukca_chem == 13:
                if not (n3 == 3 and l_ukca_new_emiss):
                    l_mask = False
                    msg = _form_msg("n3", n3, "Regional air quality")
            elif int(iopn[1:28]) > 0 or int(iopn[28:30]) > 0:
                l_mask = False
                msg = "Sec 50, 52: unknown option code"
        elif sum_iopn > 0:
            l_mask = False
            msg = "Sec 50, 52: unknown option code"
...
def _get_hvers(config, n_internal_model_max, nsectp=99):
    ...
    # A51 UKCA diagnostics on pressure levels
    if l_ukca_mode:
        l_ukca_chem_plev = _get_var(config, "namelist:run_ukca",
                  "l_ukca_chem_plev", default=False, type_="logical")
        if l_ukca_chem_plev:
            h_vers[atmos_im - 1][51] = 1

    # A52 UKCA ASAD on pressure levels
    if l_ukca_mode:
        l_ukca_asad_plev = _get_var(config, "namelist:run_ukca",
                  "l_ukca_asad_plev", default=False, type_="logical")
    ...
    l_idealised_data = _get_var(config, "namelist:run_dyntest",
                         "l_idealised_data", default=False, type_="logical")
    if l_idealised_data:
