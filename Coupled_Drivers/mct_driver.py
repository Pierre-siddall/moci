...
def _setup_rmp_dir(mct_envar, run_info):
    ...
    if run_info["l_namcouple"]:
        ...
        if len(core_dirs) < 1:
            sys.stderr.write(
                "[FAIL] failed to find core remapping " "directory %s\n" % core_dir_str
            )
        ...
        for comp1 in comp_list:
            for comp2 in comp_list:
                ...
                if not grid1 in run_info or not grid2 in run_info:
                    sys.stderr.write(
                        "[FAIL] either %s or %s is missing "
                        "from run_info.\n" % (grid1, grid2)
                    )
                    sys.exit(error.MISSING_GRID_IN_RUN_INFO)
                rmp_dir = (
                    mct_envar["RMP_DIR"] + "/" + run_info[grid2] + "_" + run_info[grid1]
                )
                # Check that directory exists
                if not os.path.isdir(rmp_dir):
                    sys.stderr.write(
                        "[FAIL] failed to find remapping " "directory %s\n." % rmp_dir
                    )
...
def _setup_executable(common_env, envarinsts, run_info):
    ...
    files_to_tidy = _multiglob(
        "nout.*", "debug.*root.*", "debug.??.*", "debug.???.*", "*fort*", "rmp_*"
    )
    for f_to_tidy in files_to_tidy:
        common.remove_file(f_to_tidy)
    ...
    if run_info["l_namcouple"]:
        ...
        if not os.path.exists("namcouple"):
            sys.stderr.write(
                "[FAIL] Could not find a namcouple file in the"
    ...
    for component in mct_envar["COUPLING_COMPONENTS"].split():
        if not component in common_env["models"]:
            sys.stderr.write(
                "[FAIL] Attempting to couple component %s,"
        ...
        if not component in list(SUPPORTED_MODELS.keys()):
            sys.stderr.write(
                "[FAIL] The component %s is not supported by the"
                " mct driver\n" % component
            )
            sys.exit(999)
        # Setup coupling for individual component
        sys.stdout.write("[INFO] MCT driver setting up %s component\n" % component)
        SUPPORTED_MODELS[component](common_env, mct_envar, envarinsts[component])
...
def _sent_coupling_fields(mct_envar, run_info):
    ...
    couple_freqs = {
        ...
        "ATM2JNR_freq": ["oasis_couple_freq_aj", "oasis_couple_freq_aj_stats"],
        "JNR2ATM_freq": ["oasis_couple_freq_ja", "oasis_couple_freq_ja_stats"],
    ...
    for component1 in mct_envar["COUPLING_COMPONENTS"].split():
        for component2 in mct_envar["COUPLING_COMPONENTS"].split():
            if component2 != component1:
                ...
                if (
                    not component1 in component_names
                    or not component2 in component_names
                ):
                    sys.stderr.write(
                        "[FAIL] %s or %s is unrecognised as "
                ...
                if "coupling_control" not in shared_nml:
                    sys.stderr.write(
                        "[FAIL] failed to find coupling_control "
                        "in SHARED namelist.\n"
                    )
                ...
                for nml_cpl_entry in nml_cpl_vars:
                    ...
                    if not cpl_var in run_info:
                        run_info[cpl_var] = []
                    cpl_freq = (
                        3600 * shared_nml["coupling_control"][nml_cpl_entry][0]
