...
def get_component_resolution(nlist_file, resolution_variables):
    ...
    for res_var in resolution_variables:
        _, out = shellout._exec_subprocess(
            "grep %s %s" % (res_var, nlist_file), verbose=True
        )
        try:
...
def get_glob_usage(glob_path, timeout=60):
    ...
    if filelist:
        ...
        sys.stderr.write(
            "[WARN] Attepting to find the size of files described"
            " by glob expression %s. There are no files found" % glob_path
        )
        size_k = 0.0
    return size_k
...
def get_datam_output_runonly(common_envar, cpmip_envar, timeout):
    ...
    if "um" in common_envar["models"]:
        um_path_gl = os.path.join(common_envar["DATAM"], "%s*" % common_envar["RUNID"])
    ...
    if "jnr" in common_envar["models"]:
        jnr_path_gl = os.path.join(
            common_envar["DATAM"], "%s*" % cpmip_envar["RUNID_JNR"]
        )
    ...
    if failed_components:
        for failed_component in failed_components:
            sys.stderr.write(
                "[FAIL] Unable to determine the usage in DATAM"
                " for the %s component\n" % failed_component
            )
...
def get_workdir_netcdf_output(timeout=60):
    ...
    output_files = [
        i_f
        for i_f in os.listdir(".")
        if i_f.split(".")[-1] == "nc" and not os.path.islink(i_f)
    ]
    size_k = -1.0
...
def seconds_to_days(time_secs):
    ...
    """
    time_days = time_secs / (24.0 * 3600.0)
    return time_days
