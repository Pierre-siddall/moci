...
class TestPrivateMethods(unittest.TestCase):
    ...
    def test_setup_dates(self, mock_exec):
        ...
        self.assertIn(
            mock.call('isodatetime 19790901T0000Z -f "%Y-%m-%d %H:%M:%S"'),
            mock_exec.mock_calls,
        )
        self.assertIn(
            mock.call(
                'isodatetime 19790901T0000Z -f "%Y-%m-%d %H:%M:%S" -s P1Y4M10DT0H0M --calendar gregorian'
            ),
            mock_exec.mock_calls,
        )
    ...
    def test_update_river_nl(self, mock_lib, mock_path, mock_shellout, mock_common):
        ...
        rivers_driver._update_river_nl(RIVER_ENV, "19790901T0000Z", "19810121T0000Z")
        ...
        self.assertIn(mock.call().var_val("output_start", "19790901T0000Z"), nml_calls)
        self.assertIn(mock.call().replace(), nml_calls)

        self.assertIn(
            mock.call().var_val("main_run_start", "19790901T0000Z"), nml_calls
        )
        self.assertIn(mock.call().var_val("main_run_end", "19810121T0000Z"), nml_calls)
        ...
        mock_lib.Path().mkdir.assert_called_once_with(parents=True, exist_ok=True)
    ...
    def test_sent_coupling_fields(self, mock_nml, mock_res, mock_path):
        ...
        second value"""
        run_info = {"exec_list": ["toyriv"], "river_nl": "rivers_coupling.nml"}
        ...
        mock_res.assert_called_once_with("model_grid.nml", {"exec_list": ["toyriv"]})
        mock_nml.read.assert_called_once_with("OASIS_RIV_SEND")
        mock_namcpl.add_to_cpl_list.assert_called_once_with("RIV", False, 0, "fields")
...
class TestSetupExecutable(unittest.TestCase):
    ...
    def test_setup_executable(self, mock_upd, mock_date, mock_link, mock_cmn, mock_env):
    ...
    def test_launcher_command(self):
        ...
        self.assertEqual(env["ROSE_LAUNCHER_PREOPTS_RIVER"], "'river pre-opts'")
