...
def _print_run_info(run_info):
    ...
    if "ATM_grid" in run_info:
        sys.stdout.write(
            "[INFO] Atmosphere:        %s (%d, %d)\n"
            % (run_info["ATM_grid"], run_info["ATM_resol"][0], run_info["ATM_resol"][1])
        )
    if "JNR_grid" in run_info:
        sys.stdout.write(
            "[INFO] Junior atmosphere: %s (%d, %d)\n"
            % (run_info["JNR_grid"], run_info["JNR_resol"][0], run_info["JNR_resol"][1])
        )
    if "OCN_grid" in run_info:
        ...
        if "OCN_resol" in run_info:
            sys.stdout.write(
                ...
                % (
                    run_info["OCN_grid"],
                    run_info["OCN_resol"][0],
                    run_info["OCN_resol"][1],
                )
            )
        else:
            sys.stdout.write("[INFO] Ocean:             %s" % run_info["OCN_grid"])
    ...
    comp_list = [comp for comp in comp_order if "{}_resol".format(comp) in run_info]
    for component1, component2 in itertools.permutations(comp_list, r=2):
        key = component2 + "2" + component1 + "_freq"
        sys.stdout.write(
            "[INFO] %s -> %s:           %0.1f\n"
            % (component2, component1, (run_info[key][0] / 60.0))
        )
        key2 = "l_hyb_stats_" + component2 + "2" + component1
        if key2 in run_info:
            if run_info[key2]:
                sys.stdout.write(
                    "[INFO] %s -> %s for stats: %0.1f\n"
                    % (component2, component1, (run_info[key][1] / 60.0))
                )
        key = component1 + "2" + component2 + "_freq"
        sys.stdout.write(
            "[INFO] %s -> %s:           %0.1f\n"
            % (component1, component2, (run_info[key][0] / 60.0))
        )
    ...
    if "ATM_grid" in run_info:
        ...
        sys.stdout.write(
            "[INFO] Atmosphere levels:              %d\n" % run_info["ATM_model_levels"]
        )
        sys.stdout.write(
            "[INFO] Soil levels:                    %d\n" % run_info["ATM_soil_levels"]
        )
        sys.stdout.write(
            "[INFO] Number of vegetation tiles:     %d\n" % run_info["ATM_veg_tiles"]
        )
        sys.stdout.write(
            "[INFO] Number of non-vegetation tiles: %d\n"
            % run_info["ATM_non_veg_tiles"]
        )
        sys.stdout.write(
            "[INFO] STASHmaster directory:          %s\n" % run_info["STASHMASTER"]
        )
    ...
    if "rmp_create" in run_info:
        sys.stdout.write(
            "[INFO] Fields where remapping files will be " "created are:\n"
        )
        for field in run_info["rmp_create"]:
            sys.stdout.write("[INFO]    - %s\n" % field)
    sys.stdout.write("[INFO] -------- Files -------- \n")
    sys.stdout.write(
        "[INFO] File containing coupling frequencies: %s\n" % run_info["SHARED_FILE"]
    )
    if "nemo_nl" in run_info:
        sys.stdout.write(
            "[INFO] Default couplings determined from:    %s\n" % run_info["nemo_nl"]
        )
...
def _checks_on_run_info(run_info):
    ...
    if "ATM_model_levels" in run_info and "JNR_model_levels" in run_info:
        if run_info["ATM_model_levels"] != run_info["JNR_model_levels"]:
            sys.stderr.write(
                "[FAIL] model_levels for Snr (=%d) and Jnr "
                "(=%d) are different.\n"
                % (run_info["ATM_model_levels"], run_info["JNR_model_levels"])
            )
    ...
    if "ATM_soil_levels" in run_info and "JNR_soil_levels" in run_info:
        if run_info["ATM_soil_levels"] != run_info["JNR_soil_levels"]:
            sys.stderr.write(
                "[FAIL] soil levels for Snr (=%d) and Jnr "
                "(=%d) are different.\n"
                % (run_info["ATM_soil_levels"], run_info["JNR_soil_levels"])
            )
    ...
    if "l_hyb_stats_ATM2JNR" in run_info:
        if run_info["ATM2JNR_freq"][0] > run_info["ATM2JNR_freq"][1]:
            sys.stdout.write(
                "[INFO] matching the main coupling frequency "
    ...
    for cpl_entry_raw in send_list_raw:
        if cpl_entry_raw == "default":
            ...
            if isinstance(cpl_entry_raw, str) and ";" in cpl_entry_raw:
                ...
                if len(parts) < 6:
                    sys.stderr.write(
                        "[FAIL] Insufficent information in %s\n" % cpl_entry_raw
                    )
                ...
                if len(parts) > 5:
                    ...
                    if sub_parts[0] in RMP_MAPPING:
                        ...
                        sys.stderr.write(
                            "[FAIL] Don't recognise this " "mapping: %s.\n" % parts[5]
                        )
                    ...
                    if not mapping:
                        sys.stderr.write(
                            "[FAIL] Need to specify a mapping "
                            "for first entry %s\n" % cpl_entry_raw
                        )
                ...
                if len(parts) > 6:
                    ...
                    if not weighting:
                        sys.stderr.write(
                            "[FAIL] Need to specify a weighting " "for first entry.\n"
                        )
                ...
                sys.stderr.write(
                    "[FAIL] the following coupling entry looks "
...
def write_namcouple(common_env, run_info, coupling_list):
    ...
    for nam_entry in coupling_list:
        if "default" in nam_entry.name_out:
            coupling_list = default_couplings.add_default_couplings(
                run_info, coupling_list
            )
            break
    ...
    write_namcouple_header.write_namcouple_header(
        common_env, nam_file, run_info, len(coupling_list)
    )

    # Sort the coupling_list by weighting
    coupling_list = sorted(coupling_list, key=lambda nam_entry: nam_entry.weight)
