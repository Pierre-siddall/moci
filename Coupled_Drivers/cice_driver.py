...
def _verify_fix_rst(pointerfile, task_start):
    ...
    try:
        ...
        task_start_datetime = datetime.datetime.strptime(task_start, "%Y,%m,%d,%H,%M")
    ...
    if not os.path.isfile(restart_path):
        sys.stderr.write(
            "[INFO] The CICE restart file %s can not be found\n" % restart_path
        )
    ...
    restartmatch = re.search(r"\d{4}-\d{2}-\d{2}", os.path.basename(restart_path))
    ...
    if restartdate != task_start:
        ...
        msg = (
            "[WARN ]The CICE restart data does not match the "
        ...
        fixed_restart_date = "%s-%s-%s" % (
            task_start[:4],
            task_start[4:6],
            task_start[6:8],
        )
        # Swap the date in the restart path
        restart_path_fixed = restart_path.replace(
            restartmatch.group(0), fixed_restart_date
        )
        ...
        sys.stdout.write(
            "[WARN] Update pointer file %s to replace \n"
            "[WARN] restart file %s\n"
            "[WARN] with\n"
...
def _load_environment_variables(cice_envar):
    ...
    cice_envar["ATM_DATA_DIR"] = "%s:%s" % (
        cice_envar["ATM_DATA_DIR"],
        cice_envar["CICE_ATMOS_DATA"],
    )
    cice_envar["OCN_DATA_DIR"] = "%s:%s" % (
        cice_envar["OCN_DATA_DIR"],
        cice_envar["CICE_OCEAN_DATA"],
    )
...
def _setup_executable(common_env):
    ...
    days_to_start = time2days.time2days(
        run_start[0], run_start[1], run_start[2], calendar
    )
    ...
    _, step_int_val = shellout._exec_subprocess(
        "grep %s %s" % (gl_step_int_match, cice_nl)
    )
    ...
    cice_histfreq_n_val = re.findall(r"histfreq_n\s*=\s*(.*)", cice_histfreq_n_val)[0]
    ...
    if False in (
        ...
        if not os.path.ispath(cice_envar["SHARED_FNAME"]):
            sys.stderr.write(
                "[FAIL] Can not find shared namelist file %s\n"
                % cice_envar["SHARED_FNAME"]
            )
            sys.exit(error.MISSING_DRIVER_FILE_ERROR)
        if not common_env["MODELBASIS"]:
            _, modelbasis_val = shellout._exec_subprocess(
                "grep model_basis_time %s" % cice_envar["SHARED_FNAME"]
            )
            modelbasis_val = re.findall(r"model_basis_time\s*=\s*(.*)", modelbasis_val)
        ...
        if not common_env["TASKLENGTH"]:
            _, tasklength_val = shellout._exec_subprocess(
                "grep run_target_end %s" % cice_envar["SHARED_FNAME"]
            )
            tasklength_val = re.findall(r"run_target_end\s*=\s*(.*)", tasklength_val)
    ...
    if cice_envar["TASK_START_TIME"] == "unavaliable":
        ...
        cmd = (
            "rose date --print-format=%%s 19700101T00Z --offset=%s"
        ...
        seconds_since_year_start = (
            int(shellout._exec_subprocess(cmd)[1]) + last_dump_seconds
        )
    ...
    if cice_rst in (os.getcwd(), "."):
        cice_restart = os.path.join(common_env["DATAM"], cice_envar["CICE_RESTART"])
    else:
        cice_restart = os.path.join(cice_rst, cice_envar["CICE_RESTART"])
    ...
    for direc in (cice_rst, cice_hist, cice_incond):
        ...
        if (
            ...
            sys.stdout.write(
                "[INFO] This is a New Run. Renaming old CICE" " history directory\n"
            )
    ...
    cice_restart_files = [
        f for f in os.listdir(cice_rst) if re.findall(r".*i\.restart\..*", f)
    ]
    ...
    if not os.path.isfile(os.path.join(cice_rst, cice_restart_files[-1])):
        if cice_envar["CICE_START"]:
            if cice_age_rest == "true":
                ...
                _, _ = shellout._exec_subprocess(
                    "%s > %s" % (cice_envar["CICE_START"], cice_restart)
                )
                sys.stdout.write(
                    "[INFO] %s > %s" % (cice_envar["CICE_START"], cice_restart)
                )
            else:
    ...
    if cice_runtype == "continue" and common_env["DRIVERS_VERIFY_RST"] == "True":
    ...
    if common_env["CONTINUE_FROM_FAIL"] == "true":
        ...
        since_start = datetime.datetime(
            ...
            run_start[0], run_start[1], run_start[2], run_start[3], run_start[4]
        )
    ...
    mod_cicenl.var_val(
        "history_file",
        "%si.%i%s" % (common_env["RUNID"], cice_histfreq_n, cice_histfreq),
    )
    ...
    mod_cicenl.var_val("restart_file", "%si.restart" % common_env["RUNID"])
