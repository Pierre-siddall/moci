...
try:
    ...
    IMPORT_ERROR_MSG = (
        "Unable to import cf_units. Ensure the scitools module" "has been loaded first."
    )
...
def _get_ln_icebergs(nemo_nl_file):
    ...
    """
    icb_rcode, icb_val = shellout._exec_subprocess("grep ln_icebergs %s" % nemo_nl_file)
    if icb_rcode != 0:
        sys.stderr.write(
            "Unable to read ln_icebergs in &namberg namelist"
...
def _verify_nemo_rst(cyclepointstr, nemo_rst, nemo_nl, nemo_nproc, nemo_version):
    ...
    nemo processors.
    """
    restart_files = [f for f in os.listdir(nemo_rst) if "restart" in f]
    ...
    if _get_ln_icebergs(nemo_nl):
        ...
        icb_restart_files = [f for f in restart_files if re.findall(nemo_icb_regex, f)]
        ...
        if len(icb_restart_files) not in (1, nemo_nproc, nemo_nproc + 1):
            sys.stderr.write(
                "[FAIL] Unable to find iceberg restart files for"
...
def _calc_current_model_date(model_basis_time, time_step, num_steps, calendar):
    """
    ...
    model_progress_secs = cf_units.date2num(
        model_basis_time, ref_time, calendar=calendar
    ) + (time_step * num_steps)
...
def _verify_fix_rst(
    restartdate, nemo_rst, model_basis_time, time_step, num_steps, calendar
):
    """
    ...
    model_basis_datetime = datetime.datetime.strptime(model_basis_time, "%Y%m%d")
    ...
    if restartdate == model_restart_date:
        ...
        msg = (
            "[WARN] The NEMO restart data does not match the "
            " current model time\n."
        ...
        all_restart_files = [
            f for f in os.listdir(nemo_rst) if re.findall(generic_rst_regex, f)
        ]
...
def _setup_executable(common_env):
    ...
    os.symlink(nemo_envar["OCEAN_EXEC"], nemo_envar["OCEAN_LINK"])

    # Setup date variables
    nleapy, model_basis, run_start, run_length, run_days = _setup_dates(common_env)

    # NEMO model setup
    if int(nemo_envar["NEMO_VERSION"]) < 306:
        sys.stderr.write(
            "[FAIL] The python drivers are only valid for nemo"
    ...
    for direc in restart_direcs:
        ...
        if (
            os.path.isdir(direc)
            and (direc not in ("./", "."))
            and common_env["CONTINUE"] == "false"
        ):
            sys.stdout.write("[INFO] directory is %s\n" % direc)
            sys.stdout.write(
                "[INFO] This is a New Run. Renaming old NEMO" " history directory\n"
            )
            ...
            if (
                common_env["SEASONAL"] == "True"
                and int(common_env["CYLC_TASK_TRY_NUMBER"]) > 1
            ):
    ...
    nemo_restart_files = [
        f
        for f in os.listdir(nemo_rst)
        if re.findall(r".+_\d{8}_restart(_\d+)?\.nc", f)
        or re.findall(r".+_\d{8}_restart_icb(_\d+)?\.nc", f)
    ]
    ...
    if not os.path.isfile(history_nemo_nl):
        sys.stderr.write(
            "[FAIL] Cannot find namelist file %s to extract "
            "timestep data.\n" % history_nemo_nl
        )
        sys.exit(error.MISSING_MODEL_FILE_ERROR)

    # First timestep of the previous cycle
    _, first_step_val = shellout._exec_subprocess(
        "grep %s %s" % (gl_first_step_match, history_nemo_nl)
    )

    nemo_first_step = int(re.findall(r".+=(.+),", first_step_val)[0])

    # Last timestep of the previous cycle
    _, last_step_val = shellout._exec_subprocess(
        "grep %s %s" % (gl_last_step_match, history_nemo_nl)
    )
    ...
    _, nemo_step_int_val = shellout._exec_subprocess(
        "grep %s %s" % (gl_step_int_match, nemo_envar["NEMO_NL"])
    )
    ...
    if os.path.isfile(latest_nemo_dump):
        ...
        if int(nemo_envar["NEMO_NPROC"]) == 1:
            sys.stderr.write(
                "[FAIL] NEMO driver does not support the running"
            ...
            if nemo_restart_count < 1:
                ...
                if os.path.isfile(nemo_rst_source):
                    sys.stdout.write(
                        "[INFO] Using rebuilt NEMO restart "
                        "file: %s\n" % nemo_rst_source
                    )
            ...
            if ice_restart_count < 1:
                ...
                if os.path.isfile(ice_rst_source):
                    sys.stdout.write(
                        "[INFO] Using rebuilt ice restart "
                        "file: %s\n" % ice_rst_source
                    )
            ...
            if iceberg_restart_count < 1:
                ...
                if os.path.isfile(iceberg_rst_source):
                    sys.stdout.write(
                        "[INFO] Using rebuilt iceberg restart"
                        "file: %s\n" % iceberg_rst_source
                    )
        ...
        if nemo_rst_date_bool:
            ...
            sys.stdout.write(
                "[INFO] Nemo has previously completed %i days\n" % completed_days
            )
        ln_restart = ".true."
        restart_ctl = 2
        ...
        if nemo_envar["NEMO_START"] != "":
            if os.path.isfile(nemo_envar["NEMO_START"]):
            ...
            elif os.path.isfile("%s_0000.nc" % nemo_envar["NEMO_START"]):
                for fname in glob.glob("%s_????.nc" % nemo_envar["NEMO_START"]):
                ...
                ln_restart = ".true."
            elif os.path.isfile("%s_0000.nc" % nemo_envar["NEMO_START"][:-3]):
                for fname in glob.glob("%s_????.nc" % nemo_envar["NEMO_START"][:-3]):
                    proc_number = fname.split(".")[-2][-4:]
        ...
        if nemo_envar["NEMO_ICEBERGS_START"] != "":
            ...
            if os.path.isfile(nemo_envar["NEMO_ICEBERGS_START"]):
                ...
                os.symlink(nemo_envar["NEMO_ICEBERGS_START"], iceberg_rst_file)
            elif os.path.isfile("%s_0000.nc" % nemo_envar["NEMO_ICEBERGS_START"]):
                for fname in glob.glob(
                    "%s_????.nc" % nemo_envar["NEMO_ICEBERGS_START"]
                ):
                    proc_number = fname.split(".")[-2][-4:]
                    ...
                    # an iceberg restart file link set up, and if there is, get
                    # rid of it because symlink wont work otherwise!
                    common.remove_file("%s_%s.nc" % (iceberg_rst_link, proc_number))

                    os.symlink(fname, "%s_%s.nc" % (iceberg_rst_file, proc_number))
            elif os.path.isfile("%s_0000.nc" % nemo_envar["NEMO_ICEBERGS_START"][:-3]):
                for fname in glob.glob(
                    "%s_????.nc" % nemo_envar["NEMO_ICEBERGS_START"][:-3]
                ):
                    proc_number = fname.split(".")[-2][-4:]
                    ...
                    os.symlink(fname, "%s_%s.nc" % (iceberg_rst_link, proc_number))
            else:
                sys.stderr.write(
            ...
            sys.stdout.write(
                "[WARN] NEMO_ICEBERGS_START not set or file(s)"
    ...
    if common_env["CONTINUE_FROM_FAIL"] == "true":
        ...
        run_start_dt = datetime.datetime(
            run_start[0], run_start[1], run_start[2], run_start[3]
        )
        model_basis_dt = datetime.datetime(
            model_basis[0], model_basis[1], model_basis[2], model_basis[3]
        )
        nemo_init_step = (run_start_dt - model_basis_dt).total_seconds() / nemo_step_int
        tot_runlen_sec = (
            run_days * 86400 + run_length[3] * 3600 + run_length[4] * 60 + run_length[5]
        )
        ...
        steps_per_hour = 3600.0 / nemo_step_int
    ...
    if int(nemo_envar["NEMO_VERSION"]) >= 400:
        ...
        update_nl_cmd = (
            "--file %s --runid %so --restart %s --restart_ctl %s"
    ...
    if ("T" in nemo_envar["L_OCN_PASS_TRC"]) or ("t" in nemo_envar["L_OCN_PASS_TRC"]):

        sys.stdout.write("[INFO] nemo_driver: Passive tracer code is " "active.\n")

        controller_mode = "run_controller"

        top_controller.run_controller(
            common_env,
            restart_ctl,
            int(nemo_envar["NEMO_NPROC"]),
            common_env["RUNID"],
...
def _set_launcher_command(launcher, nemo_envar):
    ...
    if nemo_envar["ROSE_LAUNCHER_PREOPTS_NEMO"] == "unset":
        ...
        nemo_envar["ROSE_LAUNCHER_PREOPTS_NEMO"] = (
            common.set_aprun_options(
                nemo_envar["NEMO_NPROC"],
                nemo_envar["OCEAN_NODES"],
                nemo_envar["OMPTHR_OCN"],
                nemo_envar["OHYPERTHREADS"],
                ss,
            )
            if launcher == "aprun"
            else ""
...
def get_ocean_resol(nemo_nl_file, run_info):
    ...
    if "jpiglo" in ocean_nml["namcfg"]:
        ...
        if (
            "jpiglo" not in ocean_nml["namcfg"]
            or "jpjglo" not in ocean_nml["namcfg"]
            or "cp_cfg" not in ocean_nml["namcfg"]
            or "jp_cfg" not in ocean_nml["namcfg"]
        ):
            sys.stderr.write(
                "[FAIL] cp_cfg, jp_cfg, jpiglo or jpjglo are "
        ...
        if ocean_nml["namcfg"]["cp_cfg"] != "orca":
            sys.stderr.write("[FAIL] we can currently only handle the " "ORCA grid\n")
        ...
        if "OCN_grid" not in run_info:
            sys.stderr.write(
                "[FAIL] it is necessary to declare the ocean "
            ...
            if run_info["OCN_grid"] not in OCEAN_RESOLS:
                sys.stderr.write(
                    "[FAIL] the ocean resolution for %s is "
                    "unknown" % run_info["OCN_grid"]
                )
                sys.exit(error.UNKNOWN_OCN_RESOL)
            else:
                run_info["OCN_resol"] = [
                    OCEAN_RESOLS[run_info["OCN_grid"]][0],
...
def _sent_coupling_fields(nemo_envar, run_info):
    ...
    if "oasis_ocn_send_nml" not in oasis_nml:
        sys.stderr.write(
            "[FAIL] namelist oasis_ocn_send_nml is " "missing from OASIS_OCN_SEND.\n"
        )
        sys.exit(error.MISSING_OASIS_OCN_SEND_NML)
    if "oasis_ocn_send" not in oasis_nml["oasis_ocn_send_nml"]:
        sys.stderr.write(
            "[FAIL] entry oasis_ocn_send is missing "
    ...
    model_snd_list = write_namcouple.add_to_cpl_list(
        "OCN", False, 0, oasis_nml["oasis_ocn_send_nml"]["oasis_ocn_send"]
    )
...
def write_ocean_out_to_stdout():
    ...
    for nemo_output_file in (
        ...
        if os.path.isfile(nemo_output_file):
            ...
            sys.stdout.write(
                "[INFO] Nemo output file %s not avaliable\n" % nemo_output_file
            )
...
def _finalize_executable(common_env):
    ...
    _, error_count = shellout._exec_subprocess('grep "E R R O R" ocean.output | wc -l')
    ...
    if os.path.isdir(nemo_rst) and os.path.isfile(nemo_envar_fin["NEMO_NL"]):
...
def run_driver(common_env, mode, run_info):
    ...
    if mode == "run_driver":
        ...
        if run_info["l_namcouple"]:
            ...
            run_info, model_snd_list = _sent_coupling_fields(exe_envar, run_info)
    elif mode == "finalize":
