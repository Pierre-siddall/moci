...
def find_previous_workdir(cyclepoint, workdir, taskname, task_param_run=None):
    ...
    if task_param_run:
        ...
        if not os.path.isdir(previous_workdir):
            sys.stderr.write(
                "[FAIL] Can not find previous work directory for"
                " task %s\n" % taskname
            )
...
def open_text_file(name, mode):
    ...
    if mode not in list(modes.keys()):
        ...
        for k in modes:
            options += "  %s: %s\n" % (k, modes[k])
        sys.stderr.write(
            "[FAIL] Attempting to open file %s, do not recognise"
    ...
    try:
        ...
        sys.stderr.write(
            "[FAIL] Unable to open file %s using mode %s (%s)\n"
...
def setup_runtime(common_env):
    ...
    if not common_env["CALENDAR"]:
        sys.stderr.write(
            "[WARN] setup_runtime: Environment variable"
        ...
        if calendar == "360day":
            ...
            sys.stderr.write(
                "[FAIL] setup_runtime: Calendar type %s not" " recognised\n" % calendar
            )
...
def _sort_hist_dirs_by_date(dir_list):
    ...
    try:
        dir_list.sort(
            key=lambda dname: datetime.datetime.strptime(
...
def remove_latest_hist_dir(old_hist_dir):
    ...
    history_pattern = re.sub(r"\.\d{12}", ".????????????", old_hist_dir)
    ...
    msg = (
        "[WARN] Re-attempting failed model step. \n"
