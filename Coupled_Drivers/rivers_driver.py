...
def _setup_dates(common_envar):
    ...
    start_date = "%s%s%sT%s%sZ" % (
        task_start[0].zfill(4),
    ...
    end_cmd = 'isodatetime %s -f "%s" -s %s --calendar %s' % (
        start_date,
        format_date,
        length_date,
        calendar,
...
def _setup_executable(common_envar):
    ...
    os.symlink(river_envar["RIVER_EXEC"], river_envar["RIVER_LINK"])
...
def _set_launcher_command(river_envar):
    ...
    river_envar["ROSE_LAUNCHER_PREOPTS_RIVER"] = (
        "'%s'" % river_envar["ROSE_LAUNCHER_PREOPTS_RIVER"]
    )
    return launch_cmd
...
def _get_river_resol(river_nl_file, run_info):
    ...
    if not os.path.isfile(river_nl_file):
        sys.stderr.write(
            "[FAIL] Can not find the river namelist file %s\n" % river_nl_file
        )
        sys.exit(error.MISSING_DRIVER_FILE_ERROR)
    ...
    if (
        "nx" not in river_nml["jules_input_grid"]
        or "ny" not in river_nml["jules_input_grid"]
    ):
        sys.stderr.write(
            "[FAIL] nx or ny are missing from namelist"
...
def _sent_coupling_fields(river_envar, run_info):
    ...
    if "oasis_riv_send_nml" not in oasis_nml:
        sys.stderr.write(
            "[FAIL] namelist oasis_riv_send_nml is " "missing from OASIS_RIV_SEND.\n"
        )
        sys.exit(error.MISSING_OASIS_RIV_SEND_NML)
    if "oasis_riv_send" not in oasis_nml["oasis_riv_send_nml"]:
        sys.stderr.write(
            "[FAIL] entry oasis_riv_send is missing "
