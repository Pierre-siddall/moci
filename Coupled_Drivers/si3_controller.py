...
def _check_si3nl_envar(envar_container):
    ...
    if not os.path.isfile(envar_container["SI3_NL"]):
        sys.stderr.write(
            "[FAIL] si3_controller: Can not find the SI3 namelist "
...
def _verify_si3_rst(cyclepointstr, nemo_nproc, si3_restart_files):
    ...
    current_rst_files = [f for f in si3_restart_files if re.findall(si3_rst_regex, f)]

    if len(current_rst_files) not in (1, nemo_nproc, nemo_nproc + 1):
        sys.stderr.write(
            "[FAIL] Unable to find SI3 restart files for"
...
def _setup_si3_controller(
    ...
    if common_env["CONTINUE_FROM_FAIL"] == "true":
        sys.stderr.write(
            "[FAIL] si3_controller is not coded to work with" "CONTINUE_FROM_FAIL=true"
        )
    ...
    si3_restart_files = [
        f for f in os.listdir(si3_rst) if re.findall(r".+_\d{8}_restart_ice", f)
    ]
    ...
    if common_env["CONTINUE"] == "false":
        ...
        if os.path.isfile(latest_si3_dump):
            sys.stdout.write(
                "[INFO] si3_controller: Removing old SI3 " "restart data\n\n"
            )
        ...
        if si3_envar["SI3_START"] != "":
            ...
            elif os.path.isfile("%s_0000.nc" % si3_envar["SI3_START"]):
                for fname in glob.glob("%s_????.nc" % si3_envar["SI3_START"]):
            ...
            elif os.path.isfile("%s_0000.nc" % si3_envar["SI3_START"][:-3]):
                for fname in glob.glob("%s_????.nc" % si3_envar["SI3_START"][:-3]):
                    proc_number = fname.split(".")[-2][-4:]
            ...
            sys.stdout.write(
                "[INFO] si3_controller: SI3 is starting from" " climatology.\n\n"
            )
        ...
        if si3_dump_time != nemo_dump_time:
            sys.stderr.write(
                "[FAIL] si3_controller: Mismatch in SI3 restart "
        ...
        # a CRUN.
        sys.stdout.write(
            "[INFO] si3_controller: Restart data avaliable in "
        ...
        if si3_restart_count < 1:
            ...
            if os.path.isfile(si3_rst_source):
                sys.stdout.write(
                    "[INFO] Using rebuilt SI3 restart " "file: %s\n" % si3_rst_source
                )
        ...
        sys.stderr.write(
            "[FAIL] si3_controller: No restart data avaliable in "
...
def _set_launcher_command(_):
    ...
    """
    sys.stdout.write(
        "[INFO] si3_controller: SI3 uses the same launch " "command as NEMO\n\n"
    )
    launch_cmd = ""
    return launch_cmd
...
def run_controller(
    ...
    if mode == "run_controller":
        exe_envar = _setup_si3_controller(
            common_env, restart_ctl, nemo_nproc, runid, verify_restart, nemo_dump_time
        )
