...
def _update_iodef(is_server_mode, is_coupled_mode, oasis_components, iodef_fname):
    """
    ...
    for line in iodef_file.readlines():
        ...
        if "<!" not in line and "using_server" in line:
            line = line.replace(
                text_bool[not is_server_mode], text_bool[is_server_mode]
            )
        # Update the coupled mode if the current setting is not what we want
        elif "<!" not in line and "using_oasis" in line:
            line = line.replace(
                text_bool[not is_coupled_mode], text_bool[is_coupled_mode]
            )
...
def _setup_executable(common_env):
    ...
    if xios_envar["XIOS_NPROC"] == "0":
        ...
        os.symlink(xios_envar["XIOS_EXEC"], xios_envar["XIOS_LINK"])
    ...
    _update_iodef(
        using_server, using_coupler, oasis_components, xios_envar["IODEF_FILENAME"]
    )
...
def _set_launcher_command(launcher, xios_envar):
    ...
    if xios_envar["XIOS_NPROC"] != "0":
        if xios_envar["ROSE_LAUNCHER_PREOPTS_XIOS"] == "unset":
            ...
            xios_envar["ROSE_LAUNCHER_PREOPTS_XIOS"] = (
                common.set_aprun_options(
                    xios_envar["XIOS_NPROC"],
                    xios_envar["XIOS_NODES"],
                    ompthr,
                    hyperthreads,
                    ss,
                )
        ...
        xios_envar["ROSE_LAUNCHER_PREOPTS_XIOS"] = (
            "'%s'" % xios_envar["ROSE_LAUNCHER_PREOPTS_XIOS"]
        )
    else:
        launch_cmd = ""
