...
class CylcCycle(object):
    ...
    def __init__(self, cyclepoint=None, cycleperiod=None):
        ...
        enddate = add_period_to_date(self.startcycle["intlist"], cycleperiod)
...
def finalcycle():
    ...
    if "true" in arch_final.lower():
        fcycle = True
        log_msg("ARCHIVE_FINAL=true.  End-of-run data will be archived.", level="INFO")
    else:
...
def check_directory(datadir):
    ...
    try:
        ...
        log_msg("check_directory: Exiting - No directory provided", level="FAIL")
...
def create_dir(dirname, path=None):
    ...
    try:
        ...
        if exc.errno == errno.EEXIST and os.path.isdir(dirname):
            ...
            log_msg("create_dir: Unable to create directory: " + dirname, level="FAIL")
    ...
    for fname in delfiles:
        try:
            ...
            if not ignore_non_exist:
                log_msg("remove_files: File does not exist: " + fname, level="WARN")
    ...
    for elem in delta:
        if elem != 0:
            ...
            if re.match(r"^\d{8}T\d{4}$", dateinput):
                ...
                log_msg(
                    "add_period_to_date: Invalid date for conversion to "
            ...
            if rcode == 0:
                ...
                log_msg(
                    "`{}` command failed:\n{}".format(datecmd, output), level="ERROR"
                )
                outdate = None
                break
...
def _mod_360day_calendar_date(indate, delta):
    ...
    try:
        ...
        log_msg(
            "add_period_to_date: Invalid date representation: " + str(indate),
            level="FAIL",
        )
    diff_hours = 0
    # multiplier to convert the delta list to a total number of hours
    multiplier = [360 * 24, 30 * 24, 24, 1, 1.0 / 60, 1.0 / 60 / 60]
...
def get_frequency(delta, rtn_delta=False):
    ...
    preserve_neg = None
    while delta:
        neg, iso, subdaily, delta = re.match(
            r"(-?)(p?)(t?)([\w\-]*)", delta.lower()
        ).groups()
        if subdaily:
        ...
        try:
            ...
            log_msg(
                "get_frequency - Invalid target provided: " + concatdelta, level="FAIL"
            )
...
def catch_failure():
    ...
    """
    if get_debugmode():
        log_msg("Ignoring failed external command. Continuing...", level="DEBUG")
